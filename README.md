...
- **Editing a registered entry** (`/correspondence/<pk>/edit/`) — corrects subject/sender/dates/remarks/received-via after registration. Gated to whoever registered it (or a superuser) — matches the existing Postal-Officer-gated registration pattern — and blocked once Closed, like every other mutation of the letter's own state (`department` is deliberately excluded from this form; correcting it goes through Reassign, so there's one audited path for department changes, not two). Logged as a `RoutingEvent.Action.EDIT` entry naming which fields changed.
- **A visual progress stepper** on the letter detail page (Received → In progress/Pending/With so-and-so → Closed), reflecting `Correspondence.status` — plain Bootstrap/CSS, no JS library.
